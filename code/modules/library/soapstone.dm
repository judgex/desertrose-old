/obj/item/soapstone
	name = "soapstone"
	desc = "Leave informative messages for travellers, including the those of from the future  shuttles."
...
/obj/item/soapstone/afterattack(atom/target, mob/user, proximity)
	...
	if(can_use() && do_after(user, tool_speed, target = T) && can_use()) //This looks messy but it's actually really clever!
		if(!locate(/obj/structure/chisel_message) in T)
			user.visible_message("<span class='notice'>[user] leaves a message for future travellers!</span>", "<span class='notice'>You engrave a message into [T]!</span>", "<span class='italics'>You hear a chipping sound.</span>")
			playsound(loc, 'sound/items/gavel.ogg', 50, 1, -1)
